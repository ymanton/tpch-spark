...
`tpch-spark` is written in Scala as a [self-contained Spark application](https://spark.apache.org/docs/latest/quick-start.html#self-contained-applications). The TPC-H queries are implemented as SQL files in the `src/sql` directory.
...
- By default, the SQL query files are expected in `"<current working directory>/src/sql"`. You can point to another location by setting the environment variable `TPCH_QUERY_SQL_DIR`.
